...
> 协作协议：**A2A**（经 AgentRouter）· 数据访问：**MCP** · 个人负责范围见 **[260630_实现状态.md](./260630_实现状态.md)**
...
agents:
  ...
  - name: triage-agent
    description: 异常分诊、缺陷分类、紧急度评估
    url: http://triage-svc:8001/a2a/v1
    capabilities: [defect_classification, severity]
    mcp_servers: [mes]
    enabled: false  # P1
...
| report-reporter-agent | qms, knowledge | 3 个（P1） |

完整矩阵与架构图：**[260630_MCP工具矩阵.md](./260630_MCP工具矩阵.md)** · [260630_架构A2A_MCP.md](./260630_架构A2A_MCP.md) 图 3b。
...
> **星型规则**：业务 Agent **仅**与 AgentRouter 通信（实线）；剧本中的逻辑先后见 [260630_剧本规范.md](./260630_剧本规范.md)，**不**表示 Agent 直连。  
> Mermaid 预览见 [260630_架构A2A_MCP.md](./260630_架构A2A_MCP.md) 文首兼容性说明。
...
> **8D 分工**：RCA Reporter 出草稿；**report-reporter-agent** 定稿写 QMS（见 PLAYBOOK §3.5）。

## 5. 剧本与 Agent 调用序

权威定义见 **[260630_剧本规范.md](./260630_剧本规范.md)**（6 剧本 + `task_type=rca` 快捷入口）。
